# Converted from PinModes/WS2812/WS2812_Ex04_Animation/WS2812_Ex04_Animation.ino
import SerialWombat
...
# ===== Arduino tab: WS2812_Ex04_Animation.ino =====
#
...
NUMBER_OF_FRAMES = 3
Frames = [ [SW_OFF,SW_OFF,SW_GREEN], [SW_OFF,SW_YELLOW,SW_OFF], [SW_RED,SW_OFF,SW_OFF], ]
def setup():
  ...
  offset = ws2812.readBufferSize()
  # We have a second location in the Serial Wombat chip's user buffer.  This is where
  # The animation frames are stored.  The readBufferSize() method gets the length of
  # buffer used by the configured number of LEDs.
  # Location in memory to store the animation frames, after the main WS2812 buffer
  # Number of frames
  ws2812.writeAnimationUserBufferIndex(WS2812_USER_BUFFER_INDEX + offset, NUMBER_OF_FRAMES )
  for i in range(0, NUMBER_OF_FRAMES):
    ws2812.writeAnimationFrame(i,Frames[i])
    # Transfer the frame to the animation buffer on the Serial Wombat chip
    ws2812.writeAnimationFrameDelay(i,5000)
    # Initalize All Frames 5000 mS delay
  ws2812.writeAnimationFrameDelay(1,1000)
  # Make the yellow frame (index 1 )  only 1000 mS instead of 5000.
