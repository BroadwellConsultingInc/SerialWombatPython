# Converted from Boards/PCB0031_SW8B_GRIP/GRIP_Ex04_CurrentPositionSweep/GRIP_Ex04_CurrentPositionSweep.ino
import SerialWombat
...
def setup():
  ...
  for position in range(0, (65000) + 1):
    grip.gs0.writePublicData(position)
    delay(250)
    current = grip.gs0.sensor.readAveragedCounts()
    current_mA = grip.gs0.sensor.readCurrent_mA()
    if current < minCurrent:
      minCurrent = current
      minCurrentPosition = position
    s = [0] * (50)
    s = ("Pos: %lu Current: %u counts, %d mA") % (position,current, current_mA)
    print(s)
